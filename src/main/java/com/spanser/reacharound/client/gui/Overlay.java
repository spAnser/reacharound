...
public class Overlay {
    ...
    public static void drawBox(MatrixStack matrices, VertexConsumer vertexConsumer, float x1, float y1, float z1, float x2, float y2, float z2, int color) {
        ...
        vertexConsumer.vertex(matrix4f, x1, y2, z1).color(color).next();
        vertexConsumer.vertex(matrix4f, x1, y2, z2).color(color).next();
        vertexConsumer.vertex(matrix4f, x2, y2, z2).color(color).next();
        vertexConsumer.vertex(matrix4f, x2, y2, z1).color(color).next();

        vertexConsumer.vertex(matrix4f, x1, y2, z1).color(color).next();
        vertexConsumer.vertex(matrix4f, x2, y2, z1).color(color).next();
        vertexConsumer.vertex(matrix4f, x2, y1, z1).color(color).next();
        vertexConsumer.vertex(matrix4f, x1, y1, z1).color(color).next();

        vertexConsumer.vertex(matrix4f, x2, y2, z2).color(color).next();
        vertexConsumer.vertex(matrix4f, x1, y2, z2).color(color).next();
        vertexConsumer.vertex(matrix4f, x1, y1, z2).color(color).next();
        vertexConsumer.vertex(matrix4f, x2, y1, z2).color(color).next();
