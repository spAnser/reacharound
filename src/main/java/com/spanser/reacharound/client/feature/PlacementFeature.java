...
public class PlacementFeature {
    ...
    public static void checkPlayerReacharoundTarget(ClientPlayerEntity player) {
        ...
        double range = MinecraftClient.getInstance().interactionManager.getReachDistance() - 0.5f;
    ...
    public static void keybindToggle(MinecraftClient client) {
        while (Reacharound.getInstance().keyBindingToggle.wasPressed()) {
            ...
            Text enabledText = new TranslatableText(config.enabled ? "reacharound.config.indicator.enabled" : "reacharound.config.indicator.disabled");
            client.player.sendMessage(new LiteralText("Reacharound ").append(enabledText), false);
        }
    }
